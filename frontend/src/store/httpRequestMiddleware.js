...
export default function httpRequestMiddleware({dispatch, getState}) {
    return next => action => {
        ...
        const {httpRequest, type, ...rest} = action;
        if (!httpRequest)
            return next(action);
        ...
        // return request[method].call(request, path, {params, data, files, token})
        return request[method].call(request, path, {params, data, files})

            .then(extractHttpResponse)
            .then(
                result => {
                    const parsedResult = parseResult(result);
                    const actionData = {...rest, result: parsedResult, request: httpRequest};
                    dispatchResult(next, actionData, SUCCESS);
                },
                error => {

                    if (isNotServerError(error))
                        console.error(error);

                    if (isInternalServer(error))
...
function getTypes(type) {
    return [
        extendActionType.success(type),
        extendActionType.fail(type)
    ]
}
